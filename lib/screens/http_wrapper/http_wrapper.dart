// ignore_for_file: depend_on_referenced_packages, prefer_interpolation_to_compose_strings, unrelated_type_equality_checks

import 'dart:convert';
import 'dart:io';
import 'package:aspen_project/utils/helpers/progress_hud.dart';
...
class HttpWrapper {
  ...
  HttpWrapper({
    ...
    //     CurrentSession().getUser()!.token != null) {
    //   headers = {
    //     'Content-Type': 'application/json',
    //     'accept': 'application/json',
    //     'Accept-Language': AppLocale().isArabic() ? '1' : '2',
  ...
  Future<ApiResponse?> put({required HttpWrapper wrapper}) async {
    ...
    try {
      ...
      final response = await ioClient
        ...
        return Response("S.current.generalError", 500);//localization not available
      });
  ...
  Future<ApiResponse> startMultiPartPost({required HttpWrapper wrapper}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        apiResponse.body = utf8.encode(resp);
      } else {
